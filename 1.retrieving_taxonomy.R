...
setdiff(actinopterygii_species_corrected$COMPLETE_NAMES, actinopterygii_taxid_merged$COMPLETE_NAMES)
setdiff(actinopterygii_species_corrected$COMPLETE_NAMES, actinopterygii_taxid_merged$COMPLETE_NAMES)
...
## Checking if REFSEQ sequences are duplicated

refseq_check = act_mitogenomes %>% group_by(SPECIES) %>%
  mutate(REFSEQ = any(grepl("^NC_", ACCESSION)), REFSEQ_DNA = SEQUENCES[grepl("^NC_", ACCESSION)][1], 
         REFSEQ_DNA_DUPLICATED = any(REFSEQ_DNA == SEQUENCES[!grepl("^NC_", ACCESSION)])) %>%
  filter(REFSEQ) %>% select(SPECIES, ACCESSION, SEQUENCES, REFSEQ_DNA_DUPLICATED)
refseq_check

refseq_check_both = subset(act_mitogenomes, SPECIES %in% subset(refseq_check, !REFSEQ_DNA_DUPLICATED & grepl("^NC_", ACCESSION))$SPECIES) %>% group_by(SPECIES) %>%
