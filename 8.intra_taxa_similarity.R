...
median_intra_genus_df = pivot_longer(tibble(cbind(tibble(GENUS = rownames(median_intra_genus)), tibble(as.data.frame(median_intra_genus)))), 
                                     !GENUS, names_to = "PRIMERS", values_to = "MEDIAN")
...
mad_intra_genus_df = pivot_longer(tibble(cbind(tibble(GENUS = rownames(mad_intra_genus)), tibble(as.data.frame(mad_intra_genus)))), 
                                  !GENUS, names_to = "PRIMERS", values_to = "MAD")
...
mean_intra_genus_df = pivot_longer(tibble(cbind(tibble(GENUS = rownames(mean_intra_genus)), tibble(as.data.frame(mean_intra_genus)))), 
                                   !GENUS, names_to = "PRIMERS", values_to = "MEAN")
...
sd_intra_genus_df = pivot_longer(tibble(cbind(tibble(GENUS = rownames(sd_intra_genus)), tibble(as.data.frame(sd_intra_genus)))), 
                                 !GENUS, names_to = "PRIMERS", values_to = "SD")
...
median_intra_family_df = pivot_longer(tibble(cbind(tibble(FAMILY = rownames(median_intra_family)), tibble(as.data.frame(median_intra_family)))), 
                                      !FAMILY, names_to = "PRIMERS", values_to = "MEDIAN")
...
mad_intra_family_df = pivot_longer(tibble(cbind(tibble(FAMILY = rownames(mad_intra_family)), tibble(as.data.frame(mad_intra_family)))), 
                                   !FAMILY, names_to = "PRIMERS", values_to = "MAD")
...
mean_intra_family_df = pivot_longer(tibble(cbind(tibble(FAMILY = rownames(mean_intra_family)), tibble(as.data.frame(mean_intra_family)))), 
                                    !FAMILY, names_to = "PRIMERS", values_to = "MEAN")
...
sd_intra_family_df = pivot_longer(tibble(cbind(tibble(FAMILY = rownames(sd_intra_family)), tibble(as.data.frame(sd_intra_family)))), 
                                  !FAMILY, names_to = "PRIMERS", values_to = "SD")
...
median_intra_order_df = pivot_longer(tibble(cbind(tibble(ORDER = rownames(median_intra_order)), tibble(as.data.frame(median_intra_order)))), 
                                     !ORDER, names_to = "PRIMERS", values_to = "MEDIAN")
...
mad_intra_order_df = pivot_longer(tibble(cbind(tibble(ORDER = rownames(mad_intra_order)), tibble(as.data.frame(mad_intra_order)))), 
                                  !ORDER, names_to = "PRIMERS", values_to = "MAD")
...
mean_intra_order_df = pivot_longer(tibble(cbind(tibble(ORDER = rownames(mean_intra_order)), tibble(as.data.frame(mean_intra_order)))), 
                                   !ORDER, names_to = "PRIMERS", values_to = "MEAN")
...
sd_intra_order_df = pivot_longer(tibble(cbind(tibble(ORDER = rownames(sd_intra_order)), tibble(as.data.frame(sd_intra_order)))), 
                                 !ORDER, names_to = "PRIMERS", values_to = "SD")
...
order_taxonomic_resolution = c("FishF1-R1 (COI)", "Minibar (COI)", "L14912 (CytB)", "L14841 (CytB)", "L14735c/c2 (CytB)", "FishCB (CytB)", "Teleo2 (12S)", 
...
similarity_stat_all_taxa_subset = similarity_stat_all_taxa_subset %>%
  mutate(PRIMERS_GENE = factor(paste0(PRIMERS, " (", GENE, ")"), levels = order_taxonomic_resolution))
similarity_stat_all_taxa_subset
...
mean_length_metabarcodes_modified = mean_length_metabarcodes %>%
  mutate(PRIMERS = replace(PRIMERS, which(PRIMERS %in% "Fish2b"), "Fish2b/deg"),
         PRIMERS = replace(PRIMERS, which(PRIMERS %in% "L14735c"), "L14735c/c2"),
         PRIMERS = replace(PRIMERS, which(PRIMERS %in% "FishF1-FishR1"), "FishF1-R1"))

best_cutoff_taxa_long = best_cutoff_taxa %>% pivot_longer(!LIMIT, names_to = "PRIMERS", values_to = "THRESHOLD") %>%
  left_join(mean_length_metabarcodes_modified) %>%
  mutate(PRIMERS_GENE = factor(paste0(PRIMERS, " (", GENE, ")"), levels = order_taxonomic_resolution))
best_cutoff_taxa_long

best_cutoff_taxa_graph = bind_rows(lapply(split(best_cutoff_taxa_long, best_cutoff_taxa_long$PRIMERS_GENE), function(x) 
  tibble(GRAPH = paste0(paste0(sort(x$THRESHOLD, decreasing = T), collapse = "% > "), "%"))), .id = "PRIMERS_GENE")
best_cutoff_taxa_graph$PRIMERS_GENE = factor(best_cutoff_taxa_graph$PRIMERS_GENE, levels = order_taxonomic_resolution)
best_cutoff_taxa_graph

ggplot(similarity_stat_all_taxa_subset, aes(x = TAXA, y = MEAN)) +
  facet_wrap(~PRIMERS_GENE, nrow = 4, ncol = 5) +
  ...
  scale_y_continuous(labels = function(x) paste0(x, '%'), limits = c(40, 106), minor_breaks = seq(40, 100, 5),
  ...
  geom_text(data = best_cutoff_taxa_graph, aes(label = GRAPH, x = 2.5, y = 105), 
            size = 2.5, color = "red", family = "Segoe UI Semibold")

ggsave("Graph/intra_taxa_similarity/Intra-taxa - Mean intra-taxa similarity.png", dpi = 600, 
       device = png, width = 25.53229/1.5, height = 14.39333*2/1.5, units = "cm")
...
intra_taxa_boxplot_full_stats_subset = subset(intra_taxa_boxplot_full_stats, !(PRIMERS %in% c("Fish2deg", "L14735c2"))) %>%
  mutate(PRIMERS = replace(PRIMERS, which(PRIMERS %in% "Fish2b"), "Fish2b/deg"),
         PRIMERS = replace(PRIMERS, which(PRIMERS %in% "L14735c"), "L14735c/c2"),
         PRIMERS = replace(PRIMERS, which(PRIMERS %in% "FishF1/R1"), "FishF1-R1"))

intra_taxa_boxplot_full_stats_subset = tibble(merge(mean_length_metabarcodes_modified, intra_taxa_boxplot_full_stats_subset))
intra_taxa_boxplot_full_stats_subset

intra_taxa_boxplot_full_stats_subset = intra_taxa_boxplot_full_stats_subset %>%
  mutate(PRIMERS_GENE = factor(paste0(PRIMERS, " (", GENE, ")"), levels = order_taxonomic_resolution))
intra_taxa_boxplot_full_stats_subset

intra_taxa_boxplot_full_stats_subset = intra_taxa_boxplot_full_stats_subset %>% group_by(PRIMERS, TAXA) %>% 
  mutate(FIRST_QUANTILE_PRIMERS = unique(LOW), THIRD_QUANTILE_PRIMERS = unique(TOP)) %>% ungroup()
quantile_per_primers_full_stats = intra_taxa_boxplot_full_stats_subset[,c(1,12,5,13:14)] %>% distinct()
quantile_per_primers_full_stats

subset(quantile_per_primers_full_stats, PRIMERS == "FishF1-R1")

best_cutoff_taxa_full = tibble(as.data.frame(sapply(split(quantile_per_primers_full_stats, quantile_per_primers_full_stats$PRIMERS), function(x)
  round(sort(setNames(x$THIRD_QUANTILE_PRIMERS, x$TAXA), decreasing = T)[-1] +
          (sort(setNames(x$FIRST_QUANTILE_PRIMERS, x$TAXA), decreasing = T)[-4] -
             sort(setNames(x$THIRD_QUANTILE_PRIMERS, x$TAXA), decreasing = T)[-1])/2, 1))))
...
best_cutoff_taxa_full_long = best_cutoff_taxa_full %>% pivot_longer(!LIMIT, names_to = "PRIMERS", values_to = "THRESHOLD") %>%
  ...
  left_join(mean_length_metabarcodes_modified) %>%
  mutate(PRIMERS_GENE = factor(paste0(PRIMERS, " (", GENE, ")"), levels = order_taxonomic_resolution))
best_cutoff_taxa_full_long

best_cutoff_taxa_full_graph = bind_rows(lapply(split(best_cutoff_taxa_full_long, best_cutoff_taxa_long$PRIMERS_GENE), function(x) 
  tibble(GRAPH = paste0(paste0(sort(x$THRESHOLD, decreasing = T), collapse = "% > "), "%"))), .id = "PRIMERS_GENE")
best_cutoff_taxa_full_graph$PRIMERS_GENE = factor(best_cutoff_taxa_full_graph$PRIMERS_GENE, levels = order_taxonomic_resolution)
best_cutoff_taxa_full_graph
...
ggplot(intra_taxa_boxplot_full_stats_subset) +
  ...
  geom_boxplot(data = intra_taxa_boxplot_full_stats_subset[,c(1:10, 12)][!duplicated(intra_taxa_boxplot_full_stats_subset[,c(1:10, 12)]), ], 
  ...
  theme_() + theme(plot.caption = element_text(hjust = 0.5), legend.position = "bottom", 
                   ...
                   panel.grid.major.y = element_line(size = 0.3, color = "darkgrey"),
                   panel.grid.minor.y = element_line(size = 0.3, color = "darkgrey")) +
  geom_text(data = best_cutoff_taxa_full_graph, aes(label = GRAPH, x = 2.5, y = 105), 
            size = 2.5, color = "red", family = "Segoe UI Semibold") +
