...
vsearch_pairwise_similarity = function(metabarcodes_data, infos_data = NULL, min_similarity_threshold, 
                                       ...
                                       order_col_keeped = NULL, verbose = T){
  ...
  # Verifying that a table with infos about each sequences has been provided if necessary
  if(!is.null(order_col_keeped) && is.null(keep_col_infos_data)) 
    stop('The argument "order_col_keeped" is not NULL but no columns have been provided in "keep_col_infos_data" (NULL).')
  ...
  if(!is.null(order_col_keeped))
    if(any(!(names(table(gsub("[[:alpha:]]|[_]", "", order_col_keeped))) %in% c(1,2))))
      stop('All column names in "order_col_keeped" must be pasted to the suffixes 1 or 2 depending on if it depends on the query (1) or the target (2).')
  
  # Verifying that the order of columns is in the right format
  if(!is.null(order_col_keeped))
    if(length(which(name_col_accession == gsub("[0-9]+", "", order_col_keeped))) != 2)
      warning('The columns name_col_accession must be present twice in "order_col_keeped" (e.g. "AAA1", "AAA2").')
  
  # Verifying that the order of columns is in the right format
  if(!is.null(order_col_keeped) && !is.null(keep_col_infos_data))
    if(length(which(keep_col_infos_data %in% unique(gsub("[0-9]+", "", order_col_keeped)))) != length(keep_col_infos_data)) {
      if(length(which(keep_col_infos_data %in% unique(gsub("[0-9]+", "", order_col_keeped)))) == 0)
      { stop('No columns specified in "keep_col_infos_data" were specified in "order_col_keeped".') }
      else { warning('Not the same number of columns specified in "keep_col_infos_data" were specified in "order_col_keeped".') }}
  ...
  for(i in 1:length(data_split)){
    ...
    if(nrow(similarity_file) > 0){
      ...
      if(is.null(order_col_keeped)) similarity_file = tibble::tibble(cbind(similarity_file[,ncol(similarity_file)], 
      ...
      else similarity_file = subset(similarity_file, select = c("SIMILARITY", order_col_keeped))
    ...
    else data_list[[i]] = tibble::tibble(setNames(data.frame(matrix(ncol = length(c("SIMILARITY", order_col_keeped)))),
                                                  c("SIMILARITY", order_col_keeped)))
